    ...
    try:
        ...
        if current_role == "admin":
            ...
            query = """
                SELECT a.assignmentId, a.title, a.dueDate, c.courseCode
                FROM Enrollment e
                JOIN Assignments a ON e.courseCode = a.courseCode
                JOIN Courses c ON a.courseCode = c.courseCode
            ...
            query = """
                SELECT a.assignmentId, a.title, a.dueDate, c.courseCode
                FROM Teaching t
                JOIN Assignments a ON t.courseCode = a.courseCode
                JOIN Courses c ON a.courseCode = c.courseCode
...
@assignments_bp.route("/course/<string:course_code>/my-submissions", methods=["GET"])
@jwt_required()
def get_my_submissions(course_code: str):
    connection = None
    cursor = None
    try:
        claims = get_jwt()
        ...
        cursor.execute(
            ...
            FROM Submissions s
            JOIN Assignments a ON s.assignmentId = a.assignmentId
            WHERE s.studentId = %s AND a.courseCode = %s
            """,
